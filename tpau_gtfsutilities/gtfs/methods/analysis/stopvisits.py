...
def calculate_stop_visits():
    ...
    if not gtfs.is_multiagency():
        stops_report = calculate_stop_visits_single_agency()
    else:
        stops_report = calculate_stop_visits_multi_agency()

    utilityoutput.write_or_append_to_output_csv(stops_report, 'stop_visit_report.csv', index=False)
    
    return

def calculate_stop_visits_single_agency():
...
def calculate_stop_visits_multi_agency():
    # use original so report includes stops that have been filtered by polygon or date/time
    stops = gtfs.get_table('stops', original=True)[['stop_name', 'stop_lat', 'stop_lon']]
    ...
    stop_visits = get_multi_agency_stop_visit_counts()
...
def get_multi_agency_stop_visit_counts():
